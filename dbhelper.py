...
class DBHelper:
    ...
    def add_request(self, table_id, time):
        table = self.get_table(ObjectId(table_id))
        try:
            self.db.requests.insert_one({"owner": table['owner'],
    ...
    def delete_request(self, requist_id):
        self.db.requests.remove({"_id": ObjectId(requist_id)})
